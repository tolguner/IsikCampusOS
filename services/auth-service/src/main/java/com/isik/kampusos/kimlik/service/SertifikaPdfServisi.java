...
@Service
public class SertifikaPdfServisi {
    ...
    private static final String VERIFY_PAGE_BASE_URL = "http://localhost:8080/sertifikalar/dogrula";
 
    public byte[] generateCertificate(Kullanici kullanici, SertifikaOlusturmaIstegiOlayi olay) {
        try (PDDocument document = new PDDocument();
            ...
            try (PDPageContentStream content = new PDPageContentStream(document, page)) {
                drawCertificate(document, content, pageSize, fonts, kullanici, olay);
            }
 
            document.save(outputStream);
            return outputStream.toByteArray();
        } catch (Exception e) {
            throw new RuntimeException("Sertifika PDF'i oluşturulamadı: " + e.getClass().getSimpleName() + " - " + e.getMessage(), e);
        }
    }
    ...
    private void drawCertificate(PDDocument document,
                                 ...
                                 Kullanici kullanici,
                                 SertifikaOlusturmaIstegiOlayi olay) throws Exception {
        ...
        writeCentered(content, fonts.regular(), fonts.unicode(), 12, valueOr(olay.getKulupAdi(), "Etkinliği Düzenleyen Kulüp"), centerX, height - 116, new Color(86, 95, 112));
        ...
        writeCentered(content, fonts.bold(), fonts.unicode(), 30, valueOr(kullanici.getTamAd(), kullanici.getEposta()), centerX, height - 260, new Color(34, 42, 74));
 
        String body = "FMV Işık Üniversitesi bünyesinde, " + formatEventDate(olay) + " tarihinde gerçekleştirilen \""
                + valueOr(olay.getSertifikaBasligi(), title(olay)) + "\" etkinliğine gösterdiğiniz aktif katılım ve değerli katkılarınızdan dolayı bu sertifikayı almaya hak kazandınız.";
        ...
        writeCentered(content, fonts.bold(), fonts.unicode(), 12.5f, valueOr(olay.getEtkinlikYeri(), "FMV Işık Üniversitesi"), centerX, height - 435, new Color(24, 41, 84));
        writeCentered(content, fonts.regular(), fonts.unicode(), 12, formatIssuedAt(olay.getOlusturulmaTarihi()), centerX, height - 457, new Color(86, 95, 112));
 
        float signatureY = margin + 122;
        drawSignature(content, fonts, margin + 118, signatureY,
                valueOr(olay.getKulupAdi(), "Etkinliği Düzenleyen Kulüp"),
                valueOr(olay.getKulupBaskaniAdi(), "Kulüp Başkanı"));
        drawSignature(content, fonts, width - margin - 330, signatureY, "FMV Işık Üniversitesi", "SKS Daire Başkanlığı");
 
        String verificationUrl = verificationUrl(olay);
        float qrSize = 54;
        ...
        write(content, fonts.regular(), fonts.unicode(), 8.3f, "Sertifika No: " + valueOr(olay.getSertifikaKodu(), "-"), margin + 38, margin + 52, new Color(86, 95, 112));
    }
    ...
    private String title(SertifikaOlusturmaIstegiOlayi olay) {
        if (olay.getSertifikaBasligi() != null && !olay.getSertifikaBasligi().isBlank()) {
            return olay.getSertifikaBasligi().trim();
        }
        return olay.getEtkinlikBasligi();
    }
 
    private String formatEventDate(SertifikaOlusturmaIstegiOlayi olay) {
        String value = olay.getEtkinlikTarihi();
        if (value == null || value.isBlank()) {
            value = olay.getOlusturulmaTarihi();
        }
    ...
    private String verificationUrl(SertifikaOlusturmaIstegiOlayi olay) {
        return VERIFY_PAGE_BASE_URL + "?code=" + valueOr(olay.getSertifikaKodu(), "");
    }
